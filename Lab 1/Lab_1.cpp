...
int main()
{
	...
	do {
		...
		std::cin >> mySymbol;
		if (mySymbol.length() != 1) {
			con = false;
			continue;
		}
		int c = (int)mySymbol[0];
		con = (c >= 48 && c <= 57) || (c >= 65 && c <= 90) || (c >= 97 && c <= 122); // ввод символа с проверкой
	} while (!con);
	do {
		std::cout << "Введите строку: ";
		std::cin >> myStroka;
		if (myStroka.length() > 1) {
			con = true;
		}
		else {
			con = false;
	...
	int pos = (int)myStroka.find(mySymbol);
	if (pos != -1) {
		std::cout << "Символ " << mySymbol << " занимает " << pos << " позицию!"; // если символ найден
	}
	else {
